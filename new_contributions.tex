...
For differential testing of compilers $C_1, C_2$, define the target set
$S_{\mathrm{bug}} = \{x \in \Lang{G} : C_1(x) \ne C_2(x)\}$.
Even if no single bug-triggering input has detectable probability,
$D_G(S_{\mathrm{bug}})$ can be non-negligible because the set aggregates
many inputs.  The target set is $T$-reachable iff a disagreement can be
found in $O(T(n))$ samples.
\end{example}
